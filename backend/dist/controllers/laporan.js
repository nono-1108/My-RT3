...
exports.getLaporanBulanan = exports.getLaporanKumulatif = exports.exportLaporanKasPDF = exports.exportLaporanKasExcel = void 0;
...
async function getInitialBalance(periode) {
    ...
    const allPemasukan = await prisma_1.default.pemasukan.findMany();
    const allPengeluaran = await prisma_1.default.pengeluaran.findMany();
    const allSaldoAwal = await prisma_1.default.saldoawal.findMany();
    ...
    allPemasukan.forEach(p => {
        ...
        if (!periodMap.has(ym))
            periodMap.set(ym, { pemasukan: 0, pengeluaran: 0, saldoAwal: null });
...
const exportLaporanKasExcel = async (req, res) => {
    try {
...
exports.exportLaporanKasExcel = exportLaporanKasExcel;
const exportLaporanKasPDF = async (req, res) => {
    try {
        const { periode } = req.query;
...
exports.exportLaporanKasPDF = exportLaporanKasPDF;
const MONTH_NAMES = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
function parsePeriode(periodeString) {
    const parts = periodeString.split(' ');
    if (parts.length === 2) {
...
const getLaporanKumulatif = async (req, res) => {
    try {
        const pemasukan = await prisma_1.default.pemasukan.findMany();
        const pengeluaran = await prisma_1.default.pengeluaran.findMany();
        ...
        pemasukan.forEach(p => {
            const ym = `${p.tanggal.getFullYear()}-${String(p.tanggal.getMonth() + 1).padStart(2, '0')}`;
            if (!periodMap.has(ym))
                periodMap.set(ym, { pemasukan: 0, pengeluaran: 0, saldoAwal: null, label: formatPeriode(ym) });
            periodMap.get(ym).pemasukan += Number(p.nominal);
        });
...
exports.getLaporanKumulatif = getLaporanKumulatif;
const getLaporanBulanan = async (req, res) => {
    try {
        const { periode } = req.query; // e.g. "2026-01"
        if (!periode || typeof periode !== 'string')
            return res.status(400).json({ success: false, message: 'Periode is required' });
...
exports.getLaporanBulanan = getLaporanBulanan;
