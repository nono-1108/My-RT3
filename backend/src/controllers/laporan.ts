...
async function getInitialBalance(periode?: string): Promise<number> {
  const MONTH_NAMES = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
  function parsePeriode(periodeString: string) {
  ...
  const periodMap = new Map<string, { pemasukan: number, pengeluaran: number, saldoAwal: number | null }>();
...
export const exportLaporanKasExcel = async (req: Request, res: Response) => {
  try {
...
export const exportLaporanKasPDF = async (req: Request, res: Response) => {
  try {
    const { periode } = req.query as { periode?: string };
...
function formatPeriode(yearMonth: string) {
  const [year, month] = yearMonth.split('-');
  return `${MONTH_NAMES[parseInt(month) - 1]} ${year}`;
}

export const getLaporanKumulatif = async (req: Request, res: Response) => {
  try {
    const pemasukan = await prisma.pemasukan.findMany();
    const pengeluaran = await prisma.pengeluaran.findMany();
    const saldoAwal = await prisma.saldoawal.findMany();

    const periodMap = new Map<string, { pemasukan: number, pengeluaran: number, saldoAwal: number | null, label: string }>();
...
export const getLaporanBulanan = async (req: Request, res: Response) => {
  try {
